...
{
    public class VisibilityMarker
    {
        IntersectionFinder intersectionFinder = new IntersectionFinder();
        public void MarkVisibleSegments(List<Tuple<Point, int, Point>> sortedPoints, Graphics graphics)
        {
            ...
            for (int i = 1; i < sortedPoints.Count; i++)
            {
                ...
                Point beginP = 
                    intersectionFinder.FindIntersection(sortedPoints[i - 1].Item1.X, currClosestSegment.Item1, currClosestSegment.Item3);
                Point endP = 
                    intersectionFinder.FindIntersection(sortedPoints[i].Item1.X, currClosestSegment.Item1, currClosestSegment.Item3);
        ...
        private void CheckSegment(LinkedList<Tuple<Point, int, Point>> closestSegments, Tuple<Point, int, Point> segment, Point endP)
        {
            if (segment.Item2 == 0)
            {
                if (segment.Item1.Y < endP.Y)
                    closestSegments.AddLast(segment);
                else closestSegments.AddToLinkedList(segment, intersectionFinder);
            }
            else
            {
